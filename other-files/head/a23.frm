...
Begin VB.Form a23 
   ...
   Begin VB.Label Label1 
      BackColor       =   &H0080FFFF&
      Caption         =   "   猜一个数字，和两个人工智能一起玩。看谁最先猜中。"
      BeginProperty Font 
         Name            =   "宋体"
         Size            =   12
         Charset         =   134
...
If guess > min And guess < max Then
    ...
    ElseIf guess = x Then
        ...
        If liansheng = 1 Then
            jf = jf + 1
            savedj
            MsgBox "很遗憾,你猜对了!!!(你妹的RP真好!)" & vbCrLf & "你获得了1积分", 64, "消息"
        Else
            jf = jf + liansheng
            savedj
            MsgBox "很遗憾,你猜对了!!!(你妹的RP真好!)" & vbCrLf & liansheng & "连胜！你获得了" & liansheng & "积分！", 64, "很遗憾OAO"
        End If
        l33 = Val(l33) + 1
        min = 0
        max = 100
...
If guess > x Then
    MsgBox guess & vbCrLf & "太大了", , "人工智能1猜的数字为："
    max = guess
    l4 = max
ElseIf guess < x Then
    MsgBox guess & vbCrLf & "太小了", , "人工智能1猜的数字为："
    min = guess
    l3 = min
ElseIf guess = x Then
    MsgBox "人工智能1猜的数字为：" & guess & vbCrLf & "人工智能1猜对了！" & vbCrLf & "人工智能1得了一分！", 48, "恭喜你OwO"
    liansheng = 0
    l11 = Val(l11) + 1
    min = 0
    max = 100
...
If guess <= min Or guess >= max Then MsgBox "测试期间出现错误：" & vbCrLf & "人工智能猜的数字在范围外！" '错误检测
If guess > x Then
    MsgBox guess & vbCrLf & "太大了", , "人工智能2猜的数字为："
    max = guess
    l4 = max
ElseIf guess < x Then
    MsgBox guess & vbCrLf & "太小了", , "人工智能w猜的数字为："
    min = guess
    l3 = min
ElseIf guess = x Then
    MsgBox "人工智能2猜的数字为：" & guess & vbCrLf & "人工智能2猜对了！" & vbCrLf & "人工智能2得了一分！", 48, "恭喜你OwO"
    liansheng = 9
    l22 = Val(l22) + 1
    min = 0
    max = 100
